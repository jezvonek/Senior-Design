...
"""
Closest Object X
find the x coord of the potential leak source closest to the current position of the drone
...
def getClosestObjectX(x, y, z, objs):

    # ----------------------
    # Variable Instantiation
    # ----------------------

    cObj = n.zeros([1,3])                                 # preallocate the cObj array
...
def dispersion(stability, xbar):
    ...
    # compute sigma_y
    sigma[0] = 1000 * xbar * m.tan(T*pi/180.0)/2.15
...
def outline(bounds, res):
    ...
    for i in range(0, int((n.shape(wps)[0]-2)/2)):                                      # loop over the waypoints in pairs of left and right save the last pair
        ...
        wps[2*i,2] = bounds[1,3] - res*i                                                # fill in the z coordinate
        wps[2*i+1,2] = bounds[1,3] - res*i

    switcher = (n.shape(wps)[0]/2)%2                                                    # find whether there is an even or odd number of rows
    wps[n.shape(wps)[0]-2, 0] = bounds[switcher + 1, 0]                                 # fill in the last rows
    wps[n.shape(wps)[0]-2, 1] = bounds[switcher + 1, 1]
    wps[n.shape(wps)[0]-2, 0] = bounds[0, 2]
    wps[n.shape(wps)[0]-1, 0] = bounds[(switcher + 1)%2, 0]
    wps[n.shape(wps)[0]-1, 0] = bounds[(switcher + 1)%2, 1]
    wps[n.shape(wps)[0]-1, 0] = bounds[0, 2]
...
def fluxPlane(x, y, z, objs, v, theta, temp, overcast=False, res):
    
    # -----------------------------------
    # Finding the Dispersion Coefficients
    # -----------------------------------
    
    closeObj = getClosestObjectX(x,y,z,obj)             # find the coordinates of the closest potential leak source
    ...
    xhat1 = x - sigma[0]*m.sin(theta)                   # the x and y coordinates of the flux plane bounds
    yhat1 = y + sigma[0]*m.cos(theta)
    xhat2 = x + sigma[0]*m.sin(theta)
    yhat2 = y + sigma[0]*m.cos(theta)

    # z coordinates for the flux plane bounds
    if z - sigma[1] < 4.0:                              # safety check for zhat1 so that the drone maintains height above the ground 
        zhat1 = 4.0
    else:
        zhat1 = z - sigma[1]
